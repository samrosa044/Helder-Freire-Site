...
async function onRequestGet2({ request, env }) {
  ...
  try {
    ...
    const { results } = tipo && tipo !== "todos" ? await env.DB.prepare(query).bind(tipo).all() : await env.DB.prepare(query).all();
    return json(results);
  } catch (e) {
...
async function onRequestPost4({ request, env }) {
  ...
  if (acao === "rejeitar") {
    ...
    try {
      ...
      ).bind(`Cadastro #${id} rejeitado \u2014 ${pend?.tipo_imovel} de ${pend?.nome}. Motivo: ${motivo || "n\xE3o informado"}`).run();
      return json({ sucesso: true });
    } catch (e) {
...
// ../.wrangler/tmp/pages-LACgc4/functionsRoutes-0.9020986401030271.mjs
var routes = [
  {
