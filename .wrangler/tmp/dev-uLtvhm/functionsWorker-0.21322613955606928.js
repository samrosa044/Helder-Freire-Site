...
// .wrangler/tmp/pages-PkKiM5/functionsWorker-0.21322613955606928.mjs
...
__name2(autenticado, "autenticado");
...
__name2(naoAutorizado, "naoAutorizado");
...
async function onRequestGet2({ request, env }) {
  ...
  try {
    ...
    let query, result;
    if (all) {
      query = tipo && tipo !== "todos" ? "SELECT * FROM imoveis WHERE tipo = ? ORDER BY id DESC" : "SELECT * FROM imoveis ORDER BY id DESC";
      result = tipo && tipo !== "todos" ? await (env.DB || env.helder_freire_imoveis).prepare(query).bind(tipo).all() : await (env.DB || env.helder_freire_imoveis).prepare(query).all();
    } else {
      query = tipo && tipo !== "todos" ? "SELECT * FROM imoveis WHERE status = 'ativo' AND tipo = ? ORDER BY id DESC" : "SELECT * FROM imoveis WHERE status = 'ativo' ORDER BY id DESC";
      result = tipo && tipo !== "todos" ? await (env.DB || env.helder_freire_imoveis).prepare(query).bind(tipo).all() : await (env.DB || env.helder_freire_imoveis).prepare(query).all();
    }
...
__name2(verificarTurnstile, "verificarTurnstile");
async function onRequestPost2({ request, env }) {
  try {
    ...
    try {
      await DB_leads.prepare("ALTER TABLE leads ADD COLUMN cpf TEXT").run();
    } catch (_) {
    }
...
async function onRequestDelete2({ request, env }) {
  ...
  if (!DB) return json({ erro: "Banco n\xE3o configurado" }, 500);
  try {
    const url = new URL(request.url);
    const id = url.searchParams.get("id");
    if (!id) return json({ erro: "ID obrigat\xF3rio" }, 400);
    ...
    await DB.prepare("INSERT INTO auditoria (tipo, mensagem) VALUES ('delete', ?)").bind(`Lead #${id} exclu\xEDdo \u2014 ${lead?.nome || ""}`).run();
    return json({ sucesso: true });
  } catch (e) {
...
__name(verificarTurnstile2, "verificarTurnstile2");
__name2(verificarTurnstile2, "verificarTurnstile");
...
async function onRequestPost4({ request, env }) {
  ...
  try {
    ...
    try {
      await DB.prepare("ALTER TABLE pendentes ADD COLUMN cpf TEXT").run();
    } catch (_) {
    }
    try {
      await DB.prepare("ALTER TABLE pendentes ADD COLUMN cep TEXT").run();
    } catch (_) {
    }
    await DB.prepare(
...
async function onRequestDelete3({ request, env }) {
  ...
  try {
    ...
    await DB.prepare("INSERT INTO auditoria (tipo, mensagem) VALUES ('delete', ?)").bind(`Cadastro #${id} exclu\xEDdo \u2014 ${pend?.tipo_imovel || ""} de ${pend?.nome || ""}`).run();
    return json({ sucesso: true });
  } catch (e) {
    return json({ erro: "Erro ao excluir: " + e.message }, 500);
  }
...
// .wrangler/tmp/bundle-9KOgZr/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-9KOgZr/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.21322613955606928.js.map
