...
async function renderCatalogo(filtro = 'todos') {
  ...
  try {
    ...
    grid.innerHTML = imoveis.map(im => `
      <div class="pcard" onclick="openImovelModal(${JSON.stringify(im).replace(/</g,'\u003c').replace(/"/g,'&quot;')})" style="cursor:pointer">
        <div class="pcard-img">
        ...
        <div class="pcard-body">
          ...
          <div style="margin-top:10px">
            <span style="font-size:10px;color:var(--navy-light);font-weight:600;letter-spacing:.5px">Ver detalhes →</span>
          </div>
        </div>
      </div>`).join('');
  } catch (e) {
...
function openImovelModal(im) {
  const WA = 'https://wa.me/5535999811967';
  const TYPE_ICONS_MAP = { casa:'🏠', apartamento:'🏢', fazenda:'🌾', terreno:'📐', comercial:'🏪', aluguel:'🔑' };
  const TYPE_LABELS_MAP = { casa:'Casa', apartamento:'Apartamento', fazenda:'Fazenda/Sítio', terreno:'Terreno', comercial:'Comercial', aluguel:'Aluguel' };

  const isAluguel = im.modal === 'Locação';
  const isDestaque = im.destaque === 'Sim';

  document.getElementById('imovel-modal-icon').textContent = TYPE_ICONS_MAP[im.tipo] || '🏠';

  const badge = document.getElementById('imovel-modal-badge');
  badge.textContent = isAluguel ? 'Aluguel' : isDestaque ? '✓ Destaque' : 'Venda';
  badge.style.background = isAluguel ? '#f59e0b' : isDestaque ? '#22c55e' : 'var(--navy-light)';
  badge.style.color = 'white';

  document.getElementById('imovel-modal-cat').textContent = TYPE_LABELS_MAP[im.tipo] || im.tipo;
  document.getElementById('imovel-modal-titulo').textContent = im.titulo;
  document.getElementById('imovel-modal-loc').textContent = '📍 ' + im.cidade + (im.endereco ? ' · ' + im.endereco : '');
  document.getElementById('imovel-modal-preco').textContent = 'R$ ' + im.valor;

  const specs = [];
  if (im.quartos && im.quartos !== '-') specs.push(`<span style="font-size:12px;background:#f1f5f9;padding:4px 10px;border-radius:20px">🛏 ${im.quartos} qts</span>`);
  if (im.vagas && im.vagas !== '-') specs.push(`<span style="font-size:12px;background:#f1f5f9;padding:4px 10px;border-radius:20px">🚗 ${im.vagas} vagas</span>`);
  if (im.area) specs.push(`<span style="font-size:12px;background:#f1f5f9;padding:4px 10px;border-radius:20px">📐 ${im.area}</span>`);
  document.getElementById('imovel-modal-specs').innerHTML = specs.join('');

  const descEl = document.getElementById('imovel-modal-desc');
  if (im.descricao) {
    descEl.textContent = im.descricao;
    descEl.style.display = 'block';
  } else {
    descEl.style.display = 'none';
  }

  const msg = encodeURIComponent(`Olá Helder! Vi o imóvel "${im.titulo}" no seu site e tenho interesse. Pode me passar mais informações?`);
  document.getElementById('imovel-modal-wa').href = `${WA}?text=${msg}`;

  document.getElementById('imovelModal').classList.add('open');
  document.body.style.overflow = 'hidden';
}

function closeImovelModal() {
  document.getElementById('imovelModal').classList.remove('open');
