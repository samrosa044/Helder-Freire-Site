...
//
//  As imagens são armazenadas no R2 (privado) e servidas
//  via /api/foto/<chave> pelo próprio worker do site.
//  Não é necessária URL pública separada do bucket R2.
//
//  Requer:
...
export async function onRequestPost({ request, env }) {
  ...
  if (!R2) {
    return respJson({
      erro: 'Armazenamento R2 não configurado. Crie o bucket "helder-fotos" no dashboard Cloudflare e verifique o binding FOTOS_BUCKET no wrangler.toml.',
    }, 503);
  }

  // URL base do próprio site — imagens servidas via /api/foto/<chave>
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      if (!buffer.byteLength) { erros.push(`Arquivo vazio: ${nome}`); continue; }

      const ext = _extensao(tipo, nome);
      const key = `imoveis/${Date.now()}-${Math.random().toString(36).slice(2, 9)}.${ext}`;
      const contentType = (tipo === 'image/heic' || tipo === 'image/heif') ? 'image/jpeg' : tipo;
      ...
      // URL via worker — sem necessidade de bucket público
      urls.push(`${origin}/api/foto/${key}`);
    } catch (e) {
